...
`;
    ...
    function UsageControl({ directory, loadDirectory }) {
      ...
      const session = usage?.session?.usedPercent;
      const weekly = usage?.weekly?.usedPercent;
      const maximum = Math.max(session ?? 0, weekly ?? 0);
      const readings = [
        Number.isFinite(session) ? `S ${Math.round(session)}%` : null,

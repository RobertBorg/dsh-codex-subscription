...
import { CodexUsage, registerCodexUsageRoute } from './usage.js';
...
export function apply(ctx, config) {
  ...
  const usage = new CodexUsage(options, credentials);
  ...
  // Web is optional: the adapter remains usable in headless compositions, while
  // a webServer appearing in the composition receives the browser-safe quota route.
  ctx.inject(['webServer'], (scope) => {
    const webServer = scope.get('webServer');
    if (webServer === undefined) return;
    scope.effect(
      () => registerCodexUsageRoute(webServer, usage),
      'llm-codex: plan usage route',
    );
  });
}

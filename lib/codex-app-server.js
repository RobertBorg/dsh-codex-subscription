/**
 * Minimal JSONL client for the Codex CLI app-server refresh flow.
 *
 * This module never receives OAuth credentials. Codex reads and updates its own
 * credential store; the caller re-reads auth.json only after this process exits.
 */
 ...
 */
export function requestCodexAppServerRefresh({
  codexCommand = 'codex',
  codexHome,
  ...
  if (typeof codexHome !== 'string' || codexHome.length === 0) {
    throw new LlmError('Codex app-server 刷新缺少 CODEX_HOME。', 'INVALID_CONFIG');
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    const writeMessage = (message) => {
      ...
      child.stdin.write(`${JSON.stringify(message)}\n`, (error) => {
        if (error !== null && error !== undefined && accountResult === undefined) {
          rememberFailure('无法向 Codex app-server 发送刷新请求。', error);
        }
      });
    };
    ...
    const handleMessage = (line) => {
      ...
      if (message.error !== undefined) {
        ...
        rememberFailure(`Codex app-server 刷新失败(JSON-RPC ${code ?? 'error'}): ${detail}.${upgrade}`);
        return;
      }
      ...
      if (expectedId === 0) {
        ...
        writeMessage({ method: 'account/read', id: 1, params: { refreshToken: true } });
        return;
      }
      ...
      if (account?.type !== 'chatgpt') {
        if (account === null || account === undefined) {
          ...
          rememberFailure('Codex app-server 当前使用 API key，而不是可刷新的 ChatGPT 登录；请运行 "codex login"。');
        } else {
    ...
    const consumeStdout = (chunk) => {
      ...
      if (Buffer.byteLength(stdoutBuffer) > MAX_STDOUT_BUFFER_BYTES) {
        rememberFailure('Codex app-server 输出超过安全上限，未收到预期刷新响应。');
        return;
      }
      while (true) {
    ...
    const timer = setTimeout(() => {
      rememberFailure(
        `Codex app-server 刷新在 ${refreshTimeoutMs}ms 内未完成；请确认 Codex CLI 已安装、版本兼容且已登录。`,
      );
    }, refreshTimeoutMs);
    timer.unref?.();
    ...
    child.stdin.on('error', (error) => {
      if (accountResult === undefined) rememberFailure('Codex app-server 标准输入意外关闭。', error);
    });
    child.on('spawn', () => {
      writeMessage({
        method: 'initialize',
    ...
    child.on('close', (code, signal) => {
      ...
      if (code !== 0) {
        reject(
          new LlmError(
            `Codex app-server 刷新后未能正常退出(code=${code ?? 'null'}, signal=${signal ?? 'none'})。${diagnostic()}`,
            'AUTH',
          ),
        );
        return;
